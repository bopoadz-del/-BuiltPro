...
# Install optional ML/NLP packages (non-blocking — services degrade gracefully)
if [[ -f "requirements-ml-optional.txt" ]]; then
  pip install --no-cache-dir -r requirements-ml-optional.txt || \
    echo "WARNING: Some optional ML packages failed to install"
fi
...
if [[ "${INSTALL_BACKEND_OPTIONALS:-false}" == "true" ]]; then
  pip install --no-cache-dir -r backend/requirements-ml.txt || true
  pip install --no-cache-dir -r backend/requirements-translation.txt || true
fi
